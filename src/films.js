...
function moviesAverage(array) {
  ...
  const avgMovies = Number(scoresFilms.reduce((counter,item) => counter + item, 0)/scoresFilms.length.toFixed(2));
  return avgMovies;
}
...
function bestFilmOfYear(array, year) {
  const filtFilm = array.filter (film => film.year === year);
  let bestFilm = filtFilm.sort((a, b) => a.score - b.score);
  bestFilm.length = 1;
